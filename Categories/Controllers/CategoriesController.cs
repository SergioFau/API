...
using AA_API.Categories.Infraestructure;
using Microsoft.AspNetCore.Mvc;
using Microsoft.Extensions.Logging;

namespace AA_API.Categories.Controllers
{
    [ApiController]
    [Route("[controller]")]
...
/*
        [HttpPost (Name = "CreateCategory")]
        public ActionResult<Category> Post([FromBody] string nombre){      
           Category categoriaInsertar = new Category();
           int numeroContar = repositorio.contar();
           categoriaInsertar.id = numeroContar + 1;
           repositorio.add(categoriaInsertar);
           String url = Request.Path.ToString() + "/" + categoriaInsertar.id;
